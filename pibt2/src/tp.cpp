    ...
    for (auto a : A) {  // line 5
      if (TOKEN[a->id].size() - 1 != P->getCurrentTimestep()) {
        if (a->task != nullptr) {  // assigned agent
          tasks[a->id] = a->task;
          targets[a->id] = a->load_task ? a->task->loc_delivery : a->task->loc_pickup;
      ...
      if (!selected_tasks.empty()) {  // line 8
        ...
        auto task = *std::min_element
          (selected_tasks.begin(), selected_tasks.end(),
           ...
           { return pathDist(a->v_now, task1->loc_pickup) < pathDist(a->v_now, task2->loc_pickup); });
        ...
        info("   ", "assign task-", task->id, ": agent-", a->id, "at", a->v_now->id, ",",
        ...
        // line 13
          ...
          (std::all_of(unassigned_tasks.begin(), unassigned_tasks.end(),
                       [&] (Task* task) {
                         return task->assigned || task->loc_delivery != a->v_now;
                       })) {
        // line 14
...
void TP::updatePath2(int i, std::vector<Path>& TOKEN, Tasks& unassigned_tasks)
{
  ...
  for (auto p : P->getEndpoints()) {
     ...
     */
    bool cond1 = std::all_of(unassigned_tasks.begin(), unassigned_tasks.end(), [&] (Task* task) {
      return task->assigned || task->loc_delivery != p;
    });
    if (!cond1) continue;
...
void TP::updatePath(int i, Node* g, std::vector<Path>& TOKEN)
{
  ...
  AstarHeuristics fValue = [&](AstarNode* n) { return n->g + pathDist(n->v, g); };
  ...
  // get path
  auto path = getPathBySpaceTimeAstar
